...
export function getEnabledPaymentMethods() {
  ...
  if (PAYMENT_CONFIG.paymentMethods.creditCard.enabled) {
    enabled.push('creditCard');
  }
  if (PAYMENT_CONFIG.paymentMethods.debitCard.enabled) {
    enabled.push('debitCard');
  }
  if (PAYMENT_CONFIG.paymentMethods.mobileWallet.enabled) {
    enabled.push('mobileWallet');
  }
  if (PAYMENT_CONFIG.paymentMethods.bankTransfer.enabled) {
    enabled.push('bankTransfer');
  }
  if (PAYMENT_CONFIG.paymentMethods.installments.enabled) {
    enabled.push('installments');
  }
...
/**
 * Validate payment amount for VAT
 */
export function validateVATPaymentAmount(amount: number): { valid: boolean; error?: string } {
  const { vat } = PAYMENT_CONFIG;

  if (amount < vat.minimumAmount) {
    return { valid: false, error: `Minimum payment amount is AED ${vat.minimumAmount}` };
  }

  if (amount > vat.maximumAmount) {
    return { valid: false, error: `Maximum payment amount is AED ${vat.maximumAmount}` };
  }
...
/**
 * Validate payment amount for Corporate Tax
 */
export function validateCorporateTaxPaymentAmount(amount: number): { valid: boolean; error?: string } {
  ...
  if (amount < corporateTax.minimumAmount) {
    return { valid: false, error: `Minimum payment amount is AED ${corporateTax.minimumAmount}` };
  }

  if (amount > corporateTax.maximumAmount) {
    return { valid: false, error: `Maximum payment amount is AED ${corporateTax.maximumAmount}` };
  }
